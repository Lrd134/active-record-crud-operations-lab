...
def can_be_created_in_a_block(args = {title:  "Home Alone", release_date: 1990})
  ...
  m = Movie.create do |m|
    args[:title] == nil? ? nil : m.title=(args[:title])
    args[:release_date] == nil? ? nil : m.release_date=(args[:release_date])
    args[:director] == nil? ? nil : m.director=(args[:director])
    args[:lead] == nil? ? nil : m.lead=(args[:lead])
    args[:in_theaters] == nil? ? nil : m.in_theaters=(args[:in_theaters])
  end
end
...
def can_find_using_where_clause_and_be_sorted
  ...
  Movie.group("movies.release_date").having("movies.release_date > 2002").order(release_date: :desc)
end
